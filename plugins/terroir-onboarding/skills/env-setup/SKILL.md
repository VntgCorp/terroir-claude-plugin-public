...
when_to_use: 온보딩 진입점(onboarding-start)이 직무 확인 직후, 직무별 분기 앞에서 1회 호출한다. 사용자가 직접 "업무 도구 연결해줘", "업무 환경 셋팅", "/env-setup"을 요청할 때도 단독 실행할 수 있다. 개발 런타임 설치(Node·pnpm 등)는 이 스킬의 일이 아니다.
---
...
지금 바로 그 도구를 써야 하는 상황(사용자가 먼저 요청한 경우 등)에만 재시작을 **선택지로** 제안한다. 재시작을 택한 사용자에게는 새 세션에서 **"업무 도구 연결하고 재시작했음"** 이라고 입력하도록 안내한다 (`onboarding-start`의 "재시작 후 이어받기"가 받는다). 재시작으로 대화가 끊겨도 §1이 상태를 다시 확인해 연결된 항목은 건너뛰므로, 사용자가 진행 상황을 설명할 필요는 없다.
