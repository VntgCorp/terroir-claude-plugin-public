...
when_to_use: onboarding-start에서 "프로덕트 관련 직군 + 조직 GitHub 계정 있음"으로 분기됐을 때. 사용자가 직접 "GitHub 연결", "gh 인증", "/github-connect"를 요청할 때도 단독 실행할 수 있다. 또한 "[terroir-onboarding 이어받기]"가 포함된 재개 프롬프트(조직 계정 승인 후 담당자가 전달)를 붙여넣었을 때 이어받기 진입점으로 실행된다.
---
...
조직 계정 승인 후 담당자가 전달한 재개 프롬프트(`[terroir-onboarding 이어받기]` 포함, 문안은 `org-access-request` 스킬 §5)를 사용자가 붙여넣으면 이 스킬이 실행된다.
