#!/usr/bin/env python3
"""mesh 기계 검사 — stdlib only.

사용: python3 scripts/lint.py [storage경로]   (기본 = 현재 디렉토리)
...
def main():
    storage = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(".")
    if not storage.is_dir():
        print(f"ERROR: storage 경로 아님: {storage}")
        return 1

    pages = sorted(p for p in (storage / "wiki").glob("*.md")) if (storage / "wiki").is_dir() else []
    ...
    for p in pages:
        ...
        for src in parse_list(fm.get("sources", "")):
            if src.startswith("raw/") and not (storage / src).is_file():
