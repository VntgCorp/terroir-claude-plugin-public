...
- feedback 은 0.1.0 에서 1.0.0 으로, onboarding 은 1.0.0 에서 1.0.1 로 올랐다. 무엇이 최초 배포이고 무엇이
  Patch 인지 정한 기준 없이 PR 마다 판단했다.
...
| 다른 플러그인 | 산출물 계약 (한 플러그인이 남긴 파일·설정을 다른 플러그인이 읽는 구조) |
| 설치 환경 | 훅이 건드리는 설정 |
...
판단이 갈리면 Major 쪽으로 본다. 경계 사례 — onboarding 이 `/onboarding` 안에 마켓플레이스 자동 업데이트
활성화 단계를 더한 변경은 1.0.1 로 나갔다. 새 명령이 아니라 기존 흐름의 보강이라 Patch 로 봤다. 새 스킬이
생겼다면 Minor 였다.
...
| 모든 플러그인 같은 버전 (모노 버전) | 한 플러그인을 고쳐도 셋이 함께 올라간다. 무엇이 바뀌었는지 알 수 없고 플러그인을 따로 배포한다는 원칙과 어긋난다 |
...
- 적용 — 3 개 플러그인에 CHANGELOG 신설, onboarding 1.0.1 (RDDP-1629, PR #39).
